...
All application routes start with `/api/v1`. In the installed desktop application, renderer requests use `carelink://app/api/v1/...`; the private protocol dispatches to Fastify through `app.inject` without opening an HTTP listener. The optional `npm run dev:web` environment exposes the same routes on a loopback HTTP server. Requests and responses use JSON unless an upload/download contract explicitly states otherwise. The local demonstration implements password, one-time email-code and photo-step authentication for five synthetic clinicians. Opaque, hashed, revocable sessions determine the current doctor and patient scope. This is an actual local access-control workflow, but the email outbox and photo step are demonstration adapters rather than production email delivery, face matching or liveness detection. Structured medical-record drafts and E health/community changes persist only in the local demo database; they are not production clinical operations or external deliveries.
...
The current desktop header allowlist forwards `Accept`, `Content-Type`, `Accept-Language`, `Authorization`, `Range` and the record-draft concurrency header `If-Match`. `Authorization: Bearer …` carries the opaque local session; actor headers are deliberately discarded. `Range` supports local social-media playback, while E-module writes carry version and idempotency values in typed request bodies. Future retriable production commands still require agreed `Idempotency-Key` handling. Do not assume a new HTTP header automatically passes through the desktop bridge.

## Implemented API

| Method and path               | Owner                         | Response data            | Current behavior                                                                                            |
| ----------------------------- | ----------------------------- | ------------------------ | ----------------------------------------------------------------------------------------------------------- |
| `POST /api/v1/auth/login`     | A                             | email challenge          | Validates password and account/personnel status, applies failed-attempt lockout and sends a local demo email code |
...
| `401` / `403`                        | Missing/expired local session / authenticated but outside the permitted resource scope         | Current local enforcement; production identity-provider integration remains future work |
