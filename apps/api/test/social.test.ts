...
test('trusted moderation results notify reporter and content author once', () => {
  ...
  try {
    db.prepare(
      "DELETE FROM social_notifications WHERE kind IN ('report-upheld','content-moderated')",
    ).run();
    ...
    const report = service.createReport(
      { commandId: 'report-result-test', postId: 'POST-001', reason: 'other' },
      context,
    );
    const result = {
      eventId: 'moderation-result-test',
      reportId: report.id,
    ...
    service.receiveModerationResult(result);
    assert.equal(
      db.prepare('SELECT status FROM social_reports WHERE id=?').get(report.id)!.status,
      'upheld',
    );
    assert.equal(
      db.prepare("SELECT COUNT(*) n FROM social_notifications WHERE kind='report-upheld'").get()!.n,
      1,
    );
    assert.equal(
      db
        .prepare("SELECT COUNT(*) n FROM social_notifications WHERE kind='content-moderated'")
        .get()!.n,
      1,
    );
    assert.throws(() => service.getPost('POST-001', context), SocialNotFound);
    assert.throws(
      () => service.receiveModerationResult({ ...result, outcome: 'rejected' }),
      SocialConflict,
    );
  } finally {
    db.close();
...
test('private-message report results have no invalid post link and cannot be submitted by doctors', async () => {
  ...
  try {
    ...
    const report = service.createReport(
      { commandId: 'message-report-result', messageId: message.id, reason: 'harassment' },
      context,
    );
    service.receiveModerationResult({
      eventId: 'message-report-rejected',
      reportId: report.id,
      outcome: 'rejected',
      reviewedAt: context.now,
    });
    const notice = service
      ...
      .find((n) => n.kind === 'report-rejected' && n.createdAt === context.now)!;
    assert.equal(notice.postId, '');
    assert.equal(
      (
        await app.inject({
          method: 'POST',
          url: `/api/v1/social/reports/${report.id}/result`,
...
test('reply reactions persist counts and notify the reply author without duplicates', () => {
  ...
  try {
    ...
    db.prepare('DELETE FROM social_likes WHERE post_id=? AND identity_id=?').run(
      post.id,
      context.actorId,
    );
    db.prepare('DELETE FROM social_bookmarks WHERE post_id=? AND identity_id=?').run(
      post.id,
      context.actorId,
    );
    ...
    assert.equal(
      db
        .prepare("SELECT COUNT(*) n FROM social_notifications WHERE comment_id=? AND kind='like'")
        .get(target.id)!.n,
      1,
    );
    ...
    const removed = service.setCommentReaction(
      target.id,
      'like',
      false,
      'reply-unlike-test',
    ...
    assert.throws(
      () => service.setCommentReaction('missing', 'like', true, 'reply-missing-test', context),
      SocialNotFound,
    );
  } finally {
    db.close();
...
test('social migrations remain clinically isolated and fixtures are idempotent', () => {
  const db = openDatabase(':memory:');
  try {
    assert.equal(db.prepare('SELECT COUNT(*) count FROM schema_migrations').get()!.count, 19);
    const tables = db
