...
test('health HTTP routes persist writes, replay commands and reject stale or invalid input', async () => {
  ...
  try {
    ...
    const created = await app.inject({
      method: 'POST', url: '/api/v1/health/observations', payload: observationInput,
    });
    ...
    assert.ok(list.json().data.items.some((item: { id: string }) => item.id === created.json().data.id));
    ...
    const plan = await app.inject({ method: 'POST', url: '/api/v1/health/plans', payload: planInput });
    assert.equal(plan.statusCode, 201, plan.body);
    const stale = await app.inject({
      method: 'PATCH',
      url: `/api/v1/health/plans/${plan.json().data.id}`,
      payload: {
        commandId: 'cmd-plan-route-2', expectedVersion: 99, title: '修订计划', status: 'active',
        goals: ['复核记录'], nextReview: '2026-09-25', completionPercent: 20,
      },
    });
    ...
    const reminder = await app.inject({
      ...
      payload: {
        commandId: 'cmd-rem-route-1', patientId: 'PAT-001', channel: 'in-app',
        templateId: 'followup-demo', scheduledAt: '2026-09-16T09:00:00+08:00',
      },
    });
