...
export class SqliteHealthRepository implements HealthRepository {
  ...
  findReceipt(actorId: string, commandId: string): CommandReceipt | undefined {
    const row = this.db.prepare(
      'SELECT * FROM health_command_receipts WHERE actor_id=? AND command_id=?',
    ).get(actorId, commandId) as Row | undefined;
    return row ? mapReceipt(row) : undefined;
  }

  saveReceipt(receipt: CommandReceipt): void {
    this.db.prepare(`INSERT INTO health_command_receipts(
  ...
  overview(_context: RequestContext, patientId?: string): HealthOverview {
    ...
    ).all(params).map((row) => mapObservation(row as Row));
    const alerts = this.db.prepare(
      `SELECT * FROM health_alerts ${patientFilter} ORDER BY measured_at DESC,id DESC`,
    ).all(params).map((row) => mapAlert(row as Row));
    const carePlans = this.db.prepare(
      `SELECT * FROM care_plans ${patientFilter} ORDER BY updated_at DESC,id`,
    ).all(params).map((row) => mapPlan(row as Row));
    const reminderWhere = patientId
      ? "WHERE patient_id=:patientId AND status='planned'"
      : "WHERE status='planned'";
    const remindersPlanned = Number(
      this.db.prepare(`SELECT COUNT(*) count FROM reminder_tasks ${reminderWhere}`).get(params)!.count,
    );
    return {
      observations,
      alerts,
  ...
  listObservations(query: ObservationQuery, _context: RequestContext): Paginated<Observation> {
    ...
    const total = Number(
      this.db.prepare(`SELECT COUNT(*) count FROM health_observations WHERE ${where}`).get(params)!.count,
    );
  ...
  createObservation(record: Observation): void {
    this.db.prepare(`INSERT INTO health_observations(
      id,patient_id,metric,value,unit,measured_at,received_at,source,source_label,external_observation_id,quality_status
    ) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).run(
      record.id, record.patientId, record.metric, record.value, record.unit, record.measuredAt,
      record.receivedAt, record.source, record.sourceLabel, null, 'unreviewed',
    );
  }

  listPlans(patientId: string, _context: RequestContext): CarePlanDetail[] {
    return this.db.prepare(
      'SELECT * FROM care_plans WHERE patient_id=? ORDER BY updated_at DESC,id',
    ).all(patientId).map((row) => mapPlan(row as Row));
  }
  ...
  createPlan(plan: CarePlanDetail, version: CarePlanVersion, doctorId: string): void {
    this.db.prepare(`INSERT INTO care_plans(
      id,patient_id,doctor_id,title,status,goals_json,next_review,completion_percent,current_version,created_at,updated_at
    ) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).run(
      plan.id, plan.patientId, doctorId, plan.title, plan.status, JSON.stringify(plan.goals),
      plan.nextReview, plan.completionPercent, plan.version, plan.createdAt, plan.updatedAt,
    );
    this.insertPlanVersion(version);
  }

  updatePlan(plan: CarePlanDetail, version: CarePlanVersion): boolean {
    const result = this.db.prepare(`UPDATE care_plans SET
      title=?,status=?,goals_json=?,next_review=?,completion_percent=?,current_version=?,updated_at=?
      WHERE id=? AND current_version=?`).run(
      plan.title, plan.status, JSON.stringify(plan.goals), plan.nextReview, plan.completionPercent,
      plan.version, plan.updatedAt, plan.id, plan.version - 1,
    );
  ...
  listPlanVersions(planId: string, _context: RequestContext): CarePlanVersion[] {
    return this.db.prepare(
      'SELECT * FROM care_plan_versions WHERE plan_id=? ORDER BY version DESC',
    ).all(planId).map((row) => mapPlanVersion(row as Row));
  }
  ...
  listAssessments(patientId: string, _context: RequestContext): HealthAssessment[] {
    ...
    ).all(patientId).map((row) => mapAssessment(row as Row));
  }

  createAssessment(assessment: HealthAssessment): void {
    this.db.prepare(`INSERT INTO health_assessments(
      id,patient_id,plan_id,assessor_id,assessed_at,summary,recommendations_json,next_review
    ) VALUES(?,?,?,?,?,?,?,?)`).run(
      assessment.id, assessment.patientId, assessment.planId ?? null, assessment.assessorId,
      assessment.assessedAt, assessment.summary, JSON.stringify(assessment.recommendations),
  ...
  listReminders(patientId: string, _context: RequestContext): ReminderTask[] {
    return this.db.prepare(
      'SELECT * FROM reminder_tasks WHERE patient_id=? ORDER BY scheduled_at DESC,id DESC',
    ).all(patientId).map((row) => mapReminder(row as Row));
  }

  findReminder(id: string, _context: RequestContext): ReminderTask | undefined {
    const row = this.db.prepare('SELECT * FROM reminder_tasks WHERE id=?').get(id) as Row | undefined;
    return row ? mapReminder(row) : undefined;
  }

  createReminder(reminder: ReminderTask, consentReference?: string): void {
    this.db.prepare(`INSERT INTO reminder_tasks(
      ...
      reminder.id, reminder.patientId, reminder.planId ?? null, reminder.channel,
      reminder.templateId, reminder.scheduledAt, reminder.status, consentReference ?? null,
      null, reminder.attempts, reminder.lastError ?? null, `health-service-${reminder.id}`,
    );
  }

  updateReminder(reminder: ReminderTask, providerMessageId?: string): void {
    this.db.prepare(`UPDATE reminder_tasks SET status=?,attempts=?,last_error=?,provider_message_id=?
      WHERE id=?`).run(
      reminder.status, reminder.attempts, reminder.lastError ?? null, providerMessageId ?? null,
      reminder.id,
    );
  }

  private insertPlanVersion(version: CarePlanVersion): void {
    this.db.prepare(`INSERT INTO care_plan_versions(
      id,plan_id,version,payload_json,authored_by,created_at
    ) VALUES(?,?,?,?,?,?)`).run(
      version.id, version.planId, version.version, JSON.stringify(version.snapshot),
      version.authoredBy, version.createdAt,
    );
  }
}
...
function mapObservation(row: Row): Observation {
  return {
    ...
    metric: row.metric as Observation['metric'], value: Number(row.value), unit: String(row.unit),
    measuredAt: String(row.measured_at), receivedAt: String(row.received_at),
    source: row.source as Observation['source'], sourceLabel: String(row.source_label),
  };
}

function mapAlert(row: Row): HealthAlert {
  return {
    id: String(row.id), patientId: String(row.patient_id), patientName: '',
    metric: String(row.metric), value: String(row.value), severity: row.severity as HealthAlert['severity'],
    measuredAt: String(row.measured_at), sourceLabel: String(row.source_label),
...
function mapPlan(row: Row): CarePlanDetail {
  return {
    id: String(row.id), patientId: String(row.patient_id), patientName: '', title: String(row.title),
    status: row.status as CarePlanDetail['status'], goals: JSON.parse(String(row.goals_json)) as string[],
    nextReview: String(row.next_review), completionPercent: Number(row.completion_percent),
    version: Number(row.current_version), createdAt: String(row.created_at), updatedAt: String(row.updated_at),
  };
}

function mapPlanVersion(row: Row): CarePlanVersion {
  return {
    id: String(row.id), planId: String(row.plan_id), version: Number(row.version),
    snapshot: JSON.parse(String(row.payload_json)) as CarePlanVersion['snapshot'],
    authoredBy: String(row.authored_by), createdAt: String(row.created_at),
  };
}
...
function mapAssessment(row: Row): HealthAssessment {
  return {
    ...
    assessorId: String(row.assessor_id), assessedAt: String(row.assessed_at),
    summary: String(row.summary), recommendations: JSON.parse(String(row.recommendations_json)) as string[],
...
function mapReminder(row: Row): ReminderTask {
  return {
    id: String(row.id), patientId: String(row.patient_id),
    ...(row.plan_id ? { planId: String(row.plan_id) } : {}),
    channel: row.channel as ReminderTask['channel'], templateId: String(row.template_id),
    scheduledAt: String(row.scheduled_at), status: row.status as ReminderTask['status'],
    attempts: Number(row.attempts), ...(row.last_error ? { lastError: String(row.last_error) } : {}),
  };
}

function mapReceipt(row: Row): CommandReceipt {
  return {
    actorId: String(row.actor_id), commandId: String(row.command_id), operation: String(row.operation),
    requestDigest: String(row.request_digest), resourceId: String(row.resource_id),
    responseJson: String(row.response_json), createdAt: String(row.created_at),
  };
}
