...
export function MedicalMetricCardDialog({
  ...
  const [sourceLabel, setSourceLabel] = useState(() => t('手工录入'));
  ...
  function submit(event: FormEvent) {
    ...
    if (
      ...
      setError(t('请填写有效数值、避免重复指标，并确认已去除身份信息。'));
      return;
    }
    onAdd({
      schemaVersion: 1,
    ...
    <div className="community-dialog-backdrop community-subdialog">
      ...
      >
        <header>
          <div>
            <h2>{t('添加医学数据')}</h2>
            <small>{t('仅分享一次去标识化测量，不关联患者档案')}</small>
          </div>
        ...
        <form onSubmit={submit}>
          ...
          <div className="community-metric-rows">
            {rows.map((row) => (
              <div key={row.id}>
                ...
                <input
                  aria-label={`${t(metrics[row.code].name)} value`}
                  type="number"
                  step="any"
                  required
                  value={row.value}
                ...
                <button
                  type="button"
                  aria-label={t('删除{metric}', { metric: t(metrics[row.code].name) })}
          ...
          <footer>
            ...
            <Button type="submit">{t('添加到内容')}</Button>
          </footer>
        </form>
      </section>
    </div>
