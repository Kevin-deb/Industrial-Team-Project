...
export function AudioRecorder({
  ...
  async function start() {
    ...
    try {
      ...
      activeRecorder.onstop = () => {
        ...
        setRecording(false);
        if (blob.size > 10 * 1024 * 1024) setError(t('音频不能超过 10 MB。请缩短录音后重试。'));
      ...
      stream.current = null;
      setError(t('未获得麦克风权限。你仍可上传已有音频文件。'));
    }
  }
  ...
  return (
    <div className="community-audio-recorder">
      <div>
        <strong>{recording ? t('录音中 {time}', { time: formatTime(seconds) }) : t('录一段语音')}</strong>
        <small>{t('最长 3 分钟，发送前可以试听和删除')}</small>
      </div>
      {recording ? (
        <button type="button" onClick={() => recorder.current?.stop()} aria-label={t('停止录音')}>
          <Square size={14} /> {t('停止')}
        </button>
      ) : (
        <button
      ...
      <label className={disabled ? 'is-disabled' : ''}>
        ...
        <input
          ...
          onChange={(event) => {
            ...
            if (file?.size && file.size > 10 * 1024 * 1024) setError(t('音频不能超过 10 MB。'));
            else if (file) {
              setError('');
              onSelect(file);
            }
      ...
      <button type="button" onClick={onClose} aria-label={t('关闭录音工具')}>
        <X size={14} />
      </button>
