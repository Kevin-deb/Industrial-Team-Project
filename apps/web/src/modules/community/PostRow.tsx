    ...
    <article className="community-post-row">
      ...
      <div className="community-post-main">
        <div className="community-post-context">
          ...
          <time>{formatDate(post.lastActivityAt, { month: 'short', day: 'numeric' })}</time>
        </div>
        ...
        <div className="community-post-actions">
          <button
            ...
            aria-label={t(post.likedByMe ? '取消点赞' : '点赞')}
            className={post.likedByMe ? 'active' : ''}
          >
            <Heart size={15} fill={post.likedByMe ? 'currentColor' : 'none'} />
            {post.likeCount}
          </button>
          <Link to={`/community/posts/${post.id}`} aria-label={t('查看评论')}>
          ...
          <button
            ...
            aria-label={t(post.bookmarkedByMe ? '取消收藏' : '收藏')}
            className={post.bookmarkedByMe ? 'active' : ''}
          >
